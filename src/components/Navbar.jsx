  ...
  return (
    <div className='fixed top-0 left-0 right-0 z-20 bg-gradient-to-r from-violet-950 to-violet-800'>
      <div className='flex items-center justify-between px-16 py-1' >
        ...
        <div className={`z-10 flex flex-col md:flex-row md:gap-10 justify-center items-center text-base font-bold md:flex ${isMenuOpen ? 'absolute right-16 top-14 bg-gradient-to-r from-violet-950 to-violet-800 px-4 rounded-lg py-4 gap-5' : 'hidden'} md:block`}>
          
          {NAVLINKS.map((link, index)=>(
            <a key={link.key} className={`${activeLinkID == link.id ? 'font-extrabold' : 'font-light'}`} onClick={()=>handleLinkClick(link.id)} href={"#" + link.id} >{link.displayText}</a>
          ))}
        </div>
